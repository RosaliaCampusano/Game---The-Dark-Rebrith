...
export function initPlaying() {
  ...
  globals.particles = [];
  globals.particlesRain = [];
  globals.spritesHUD = [];
  globals.countInitPlaying = 0;
  ...
  globals.score = 0;
  globals.timer = new Timer(380, 0.5);
...
function initParticlesForMainMenu() {
  ...
  for (let i = 0; i < numParticles * 2; i++) {
    ...
    const colors = [`rgba(255, 255, 255, ${alpha})`];
    const color = colors[Math.floor(Math.random() * colors.length)];
    const width = 2;
    const height = Math.random() * (width * 4) + width;

    globals.particlesRain.push({
      xPos: x,
      yPos: 1,
      alpha,
      color,
      physics: { velocityY: Math.sin(angle) * speed, decay: 0.02 },
      height: height,
      width: width,
    });
  }
...
function initSprites() {
  if (globals.countInitPlaying === 0) {
    initPlayer();
    initPlayerhWizard();
    globals.countInitPlaying++;
  }
  initThrone();
  initPotion();
  initAttack();
...
function initPlayerhWizard() {
  ...
  const playerWizard = new Player(
    SpriteID.PLAYER_WIZARD,
    State.STILL_RIGHT_WIZARD,
    100,
    10,
    imageSet,
    frames,
...
function initOldJosephs() {
  ...
  const oldJoseph1 = new JosephMenu(
    SpriteID.OLD_JOSEPH1,
    State.RIGHT_JOSEPH,
    40,
    212,
    imageSet1,
  ...
  const oldJoseph2 = new JosephMenu(
    SpriteID.OLD_JOSEPH2,
    State.LEFT_JOSEPH,
    340,
    205,
    imageSet2,
...
function initHealthBarHUD() {
  ...
  const healthBarEmpty = new Sprite(
    SpriteID.HEALTH_BAR_EMPTY,
    State.BE,
    100,
    10,
    imageSetEmpty,
...
function initBackgroundStory() {
  ...
  const backgroundStory = new Sprite(
    SpriteID.BACKGROUND_STORY,
    State.BE,
    0,
    0,
    imageSet,
...
function initLoadJoseph() {
  ...
  const loadJoseph = new Sprite(
    SpriteID.LOAD_JOSEPH,
    State.LEFT_JOSEPH,
    250,
    130,
    imageSet,
...
function initSunLight() {
  ...
  for (let i = 0; i < numParticles; i++) {
    ...
    const sunParticles = new ParticleLight(
      xInit,
      yInit,
      radius,
      alpha,
...
function initParticlesForHighscore() {
  ...
  for (let i = 0; i < numParticles; i++) {
    ...
    const particleHighScore = new ParticleLight(xPos, yPos, radius, alpha, {
      velocity,
      acceleration: 0,
    });
...
function initLevels() {
  const mapLevel1 = new Level(
    level,
    new ImageSet(0, 0, 16, 16, 16, 16, 0, 0),
    1
  );

  const mapLevel2 = new Level(
    level2,
    new ImageSet(0, 0, 16, 16, 16, 16, 0, 0),
    2
  );
